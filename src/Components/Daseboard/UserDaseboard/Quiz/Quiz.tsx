...
const Quiz: React.FC = () => {
    ...
    return (
        <div className="max-w-4xl mx-auto">
            ...
            <div className="" >  <h1 className="text-2xl font-bold py-5 text-sky-800">Welcome today's Quiz</h1>
            <p className="text-sky-800 font-bold text-lg">Total Question {quiz.length} .</p>
        <div className="border border-gray-400 mb-7 w-72"></div></div>
                    ...
                    quiz?.map((item, questionIndex) => {
                        return (
                            <div key={questionIndex} className="grid justify-start">
                                <h1 className="text-xl font-semibold"><span>{questionIndex + 1}. </span>{item.question}</h1>
                                {
                                        ...
                                        return (
                                            <div key={optionIndex}>
                                                <input
                                                    ...
                                                    disabled={submitted}
                                                />
                                                <label className="text-lg ml-3" htmlFor={`q${questionIndex}_opt${optionIndex}`}><span>{optionIndex + 1} .</span>{option}</label>
                                            </div>
                                        );
                                    })
                                }
            ...
            <div className="max-w-2xl justify-start my-6">
                <input onClick={handleSubmit} disabled={submitted} type="submit" className="btn text-white w-full bg-sky-800 hover:bg-sky-900" value="Submit and get score" />
            </div>
            <div className="p-10 bg-slate-400 text-black text-lg font-semibold max-w-2xl rounded-md">
                {
                    submitted && (
                        <div>
                            <h1 className="text-xl bg-sky-800 rounded-md mb-2 text-white p-2">Result</h1>
                            ...
                            <div>
                                <h1 className="text-xl bg-sky-800 text-white my-2 rounded-md p-2">See the Answer List</h1>
                                {
                                    quiz.map((answer) => {
                                        return (
                                            <div key={answer._id}>
                                                <h1><span>{answer.question} : </span>{answer.answer + 1}</h1>
                                            </div>
                                        );
                                    })
                                }
                                <button onClick={handleTryAnother} className="text-xl btn bg-sky-800 text-white my-2 rounded-md p-2">Try Another</button>
                            </div>
                        </div>
                    )
                }
