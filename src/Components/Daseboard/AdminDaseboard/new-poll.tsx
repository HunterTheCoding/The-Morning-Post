...
const NewPoll: React.FC<NewPollProps> = () => {
  const [optionInputFields, setOptionInputFields] = useState<(null | string)[]>([null]);
  ...
  const isDateValid = (val: string) => {
    return new Date(val).getTime() - new Date().getTime() <= 0 ? 'Expiration date must be in the future' : undefined;
  };
       

  const onSubmit: SubmitHandler<NewPollFormData> = async ({ title, description, options, expiration }) => {
    try {
  ...
  const addNewOptionInputField = () => {
    setOptionInputFields((prevOptionInputFields) => [...prevOptionInputFields, null]);
  };
...
/>
        ...
        <fieldset className="space-y-2">
          ...
          {errors.expiration && (
            <p className="text-sm text-red-500">
              {errors.expiration.time?.message} {errors.expiration.date?.message}
            </p>
          )}
        </fieldset>
        {/* Submit Button */}
