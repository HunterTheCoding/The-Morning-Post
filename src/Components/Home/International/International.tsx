      ...
      <div className="grid md:grid-cols-6 grid-cols-1 gap-4">
          ...
          <div className="grid md:grid-cols-4 grid-cols-1 gap-4">
            <div className="col-span-3 my-3 flex">
              {InternationalData.slice(0, 1).map((news) => (
                <>
                  <div className="w-1/3 md:h-[200px] bg-gray-200 relative rounded-md">
                    <h2 className="font-bold lg:text-2xl md:text-xl text-lg  p-2">
                      {news.headline}
                    </h2>
                  ...
                  <img src={news.image} className="w-2/3 h-[200px]  rounded-md" alt="" />
                </>
              ))}
            </div>
            ...
            <Link key={news._id}  to={`/newsdetails/${news?._id}`} >
              <div className="" >
                <div className="md:h-[200px] w-full ml-4 md:ml-0 my-3 relative bg-gray-200 rounded-md">
                  <img src={news.image} className="w-full rounded-md" alt="image" />
          ...
          <div className="grid lg:grid-cols-4 md:grid-cols-2 grid-cols-1 gap-4">
            {InternationalData.slice(2, 6).map((news) => (
              <Link key={news._id} to={`/newsdetails/${news?._id}`}>
                <div
                  key={news._id}
                  className="md:h-[204px] md:my-3 bg-gray-200 relative  rounded-md"
                >
                  <img src={news.image} className="w-full  rounded-md" alt="" />
        ...
        <div className="col-span-2">
          {InternationalData.slice(0, 4).map((news) => (
            <Link key={news._id} to={`/newsdetails/${news?._id}`}>
              <div key={news._id} className="flex pl-10 md:pl-0 mt-4  rounded-md">
                <img src={news.image} className="w-1/3 h-[100px]  rounded-md" alt="" />
